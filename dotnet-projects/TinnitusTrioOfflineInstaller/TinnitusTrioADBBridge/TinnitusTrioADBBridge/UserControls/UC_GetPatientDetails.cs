...
namespace TinnitusTrioADBBridge.UserControls
{
    public partial class UC_GetPatientDetails : UserControl
    {
        ...
        private void populatePatientId()
        {
            try
            {
                var objpatientCodeBal = new TinnitusTrioADB_BAL.LicenseManager();
                var strPatientid = objpatientCodeBal.getPatientId();

                if(strPatientid=="")
                {
                    XtraMessageBox.Show(
                  "Your Licenses have been expired. Kindly contact the licensing team of Tinnitus Trio for extension of licenses.",
                  "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
