...
{
    public partial class CMTReport : DevExpress.XtraReports.UI.XtraReport
    {
        public CMTReport(string patientid, string moduletype, string indate, string outdate,string patientName)
        {
            InitializeComponent();
        ...
        public void GenerateReport(string patientid)
        {
            try
            {
                ...
                var cmtDs = cmtobj.GenerateCmtReport(patientid);

                var dt = cmtDs.Tables[0];
                var rowCnt = dt.Rows.Count;

                if (rowCnt <= 0) return;
                lblFrequency.Text = cmtDs.Tables[1].Rows[0][0].ToString();
                ...
                var sumRow = cmtDs.Tables[2].Rows[0][0].ToString() + " H-" + cmtDs.Tables[3].Rows[0][0].ToString() +
                             " M-" + cmtDs.Tables[4].Rows[0][0].ToString() + " S-";
        ...
        public void GenerateReport(string patientid, string indate, string outdate)
        {
            try
            {
                ...
                var cmtobj = new TinnitusTrioADB_BAL.ReportGenerator();
                var cmtDs = cmtobj.GeerateCmtDateWiseReport(patientid, indate, outdate);
